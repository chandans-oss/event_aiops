...
export default function TrainingPage() {
  ...
  return (
    <MainLayout>
      <div className="p-8 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 bg-background" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col gap-1 items-end">
              ...
              <Select value={timeline} onValueChange={setTimeline}>
                <SelectTrigger className="w-[160px] bg-secondary border-border font-bold h-10 text-[10px]">
                  <SelectValue placeholder="Select Data Range" />
                </SelectTrigger>
                <SelectContent className="bg-popover border-border">
                  <SelectItem value="1m" className="text-[10px] font-bold text-foreground/70 tracking-tight">Last 1 Month</SelectItem>
                  <SelectItem value="3m" className="text-[10px] font-bold text-foreground/70 tracking-tight">Last 3 Months</SelectItem>
                  <SelectItem value="6m" className="text-[10px] font-bold text-foreground/70 tracking-tight">Last 6 Months</SelectItem>
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="flex flex-col gap-1 items-end ml-2">
               <span className="text-[10px] font-black tracking-widest text-muted-foreground opacity-80">Training Scope</span>
              <Select value={deviceFilter} onValueChange={(v: any) => {
                setDeviceFilter(v);
                setSelectedTarget(SCOPE_TARGETS[v][0].label);
              }}>
                <SelectTrigger className="w-[180px] bg-secondary border-border font-bold h-10 text-[10px]">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent className="bg-popover border-border">
                  <SelectItem value="device" className="text-[10px] font-bold text-foreground/70 tracking-tight">Device Specific</SelectItem>
                  <SelectItem value="topology" className="text-[10px] font-bold text-foreground/70 tracking-tight">Topology Based</SelectItem>
                  <SelectItem value="group" className="text-[10px] font-bold text-foreground/70 tracking-tight">Device Group Based</SelectItem>
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="flex flex-col gap-1 items-end -ml-1">
              <span className="text-[10px] font-black tracking-widest text-muted-foreground opacity-80">
              ...
              <Select value={selectedTarget} onValueChange={setSelectedTarget}>
                ...
                <SelectContent className="bg-popover border-border">
                  {(SCOPE_TARGETS as any)[deviceFilter].map((t: any) => (
                    <SelectItem key={t.label} value={t.label} className="text-[10px] font-bold text-foreground/70 tracking-tight">
                      {t.label}
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            <Sheet>
              <SheetTrigger asChild>
                <Button variant="outline" size="icon" className="h-10 w-10 border-border hover:bg-secondary transition-all hover:border-primary/30 mt-5">
                  <Settings className="h-4 w-4 text-muted-foreground" />
                </Button>
              </SheetTrigger>
              <SheetContent className="w-[360px] bg-background border-l border-border p-0 sm:max-w-[360px] overflow-hidden" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
                <div className="h-full flex flex-col">
                  <div className="p-6 border-b border-border bg-gradient-to-br from-primary/10 to-transparent">
                    <SheetHeader className="space-y-3">
                      <div className="flex items-center gap-3">
                        <div className="h-9 w-9 rounded-xl bg-primary/20 flex items-center justify-center border border-primary/30 shadow-lg">
                      ...
                      {[
                        ...
                        <div key={idx} className="space-y-4">
                          <div className="flex items-center gap-2 border-b border-border pb-1.5">
                            ...
                            <h4 className="text-[9px] font-black uppercase tracking-[0.2em] text-primary">{section.title}</h4>
                          </div>
                            ...
                            {section.items.map((config, i) => (
                              <div key={i} className="group space-y-1">
                                <div className="flex items-center justify-between">
                                  ...
                                  <span className="text-[10px] font-black text-foreground tabular-nums bg-secondary px-2 py-0.5 rounded border border-border">{config.value}</span>
                                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-full">
          ...
          <div className="flex flex-col space-y-6">
            <div className="flex items-center justify-between px-2">
              ...
              <Badge variant="outline" className="text-[8px] font-bold border-primary/20 text-primary animate-pulse uppercase tracking-widest h-5">
                 Polling Active
              </Badge>
            </div>
            ...
            <div className="space-y-4">
              ...
              <Card className="p-5 bg-secondary/30 border-border space-y-4 h-fit overflow-hidden relative">
                <div className="space-y-3">
                  {BATCHES.map((batch, i) => (
                    <div key={batch.id} className="flex flex-col gap-1 first:mt-0 mt-2">
                      ...
                      <div className="h-8 bg-secondary/50 rounded-md relative flex items-center border border-border overflow-hidden p-1 shadow-inner">
                        <div
                ...
                <div className="flex flex-col items-center gap-1 opacity-40 pt-4 mt-2 border-t border-border">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2 px-1">
                {METRICS_LIST.map((metric, i) => (
                  <div key={i} className="bg-secondary/50 border border-border p-2 rounded-lg flex items-center justify-between group hover:bg-secondary/80 transition-colors">
                    ...
                    <Badge variant="outline" className="text-[7px] h-3.5 px-1.5 border-border opacity-50 font-mono">{metric.unit}</Badge>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="space-y-3 pb-8">
                {MODELS.map((model) => (
                  <Card
                    ...
                    className={cn(
                      "p-5 cursor-pointer transition-all duration-300 border-border group relative overflow-hidden",
                      modelStatuses[model.id] === 'active' ? "bg-primary/10 border-primary/40 shadow-2xl shadow-primary/10" : "bg-card/20 hover:bg-secondary"
                    )}
                  >
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <div className={cn(
                            ...
                            modelStatuses[model.id] === 'active' ? "bg-primary/20 border-primary/30" : "bg-secondary border-border group-hover:bg-primary/20"
                          )}>
                        ...
                        {modelStatuses[model.id] === 'active' ? (
                         <Badge className="bg-primary/20 text-primary border-primary/40 text-[7px] h-3.5 px-2 tracking-widest animate-pulse font-black">Training...</Badge>
                             ...
                             <Badge variant="outline" className="text-[7px] h-3.5 px-2 border-border font-black tracking-widest">{model.type}</Badge>
                         )}
                      </div>
        ...
        <div className="pt-6 border-t border-border flex items-center justify-between text-muted-foreground">
          ...
          <Button variant="outline" className="text-[9px] font-black uppercase tracking-widest border-border hover:bg-secondary h-9 px-5 opacity-50 hover:opacity-100 transition-all">
