      ...
      <div className="flex-1 overflow-y-auto scroll-smooth">
        ...
        <div className="p-6 bg-card/30 backdrop-blur border-b border-border space-y-4">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-1 space-y-4 border-r border-border/50 pr-4">
              ...
              <div className="pt-2">
                <span className="text-[10px] text-muted-foreground block mb-1">Affected Services</span>
                <div className="flex flex-wrap gap-1">
                  {clusterData.impactedAssets.slice(0, 3).map(asset => (
                    <span key={asset.id} className="px-1.5 py-0.5 rounded text-[10px] bg-secondary text-secondary-foreground border border-border">{asset.name}</span>
                  ))}
                </div>
              </div>
            </div>
        ...
        <Tabs value={currentTab} onValueChange={setCurrentTab} className="p-6 pt-2">
          ...
          <TabsContent value="summary" className="m-0 focus-visible:ring-0 animate-fade-in space-y-4">
            <RCASummary data={clusterData} onViewDetailedRCA={() => navigate(`/rca/detail/${cluster.id}`)} />
          </TabsContent>
