...
def hypothesis_scorer_best_intent(incident, intents, best_intent_id, threshold=0.3):
    ...
    if not best_intent:
        return {"intent_id": "unknown", "intent_score": 0, "hypotheses": []}
    ...
    return {
        ...
        "top_hypothesis": hypothesis_scores[0] if hypothesis_scores else None,
...
def call_ollama_api(prompt, model=OLLAMA_MODEL):
    ...
    try:
        ...
        return f"⚠️ Ollama API call failed: {e}"
    ...
    if matched_situation:
        ...
        for key in all_replacer_keys:
            ...
            if key == "top_hypothesis":
                ...
                if value == "None":
                    value = str(incident.get("signals", {}).get(device, "N/A").get(key,""))
                    # filled_stats[key] = int(float(value))
...
def planner_call(incident):
    ...
    print(incident.get('top_hypothesis',""))
    if 'crc' in incident.get('top_hypothesis',"").get("description").lower():
        dummy_response = {
    ...
    elif 'congestion' in incident.get('top_hypothesis',"").get("description").lower():
...
def historical_retriever(incident, k=1):
    ...
    # dummy_response = { "retrieved_cases":[ { "case_id":"INFRAON-ALARM-3467", "intent":"performance", "rca":"QoS misclassification", "sim_score":0.86, "remedy":"apply WAN-QoS policy" }, { "case_id":"INFRAON-ALARM-092", "intent":"performance", "rca":"Link capacity exhaustion", "sim_score":0.63 } ], "top_case":"INFRAON-ALARM-3467", "average_similarity":0.78 }
...
{incident['top_hypothesis']['hypothesis_id']} - {incident['top_hypothesis']['description']} (score={incident['prior']})
...
"""
    response = call_ollama_api(prompt)
    # print("Correlator LLM Response: ",response)
...
def extract_latest_metrics(metrics, device, resource_name, start_time, end_time):
    ...
    filtered_df = metrics[
        ...
        ((metrics["res_type"].str.lower() == "device") | (metrics["res_type"].str.lower() == "fan") | (metrics["res_name"] == resource_name)) &
        (metrics["device"] == device.strip())
    ]
...
def extract_syslog_entries(syslog, device, resource_name, start_time, end_time, incident):
    ...
    filtered_syslog = syslog[
        ...
        ((syslog["res_name"] == resource_name) | (syslog["device"] == device))
    ]
...
def trigger_agentic_flow(trigger_event, file_path='', dashboard_call=0):
    ...
    if dashboard_call==1:
        # yield incident
        yield json.dumps(incident, default=str)
    ...
    incident["intent"] = rule_best[0].split(".")[0]
    incident["subintent"] = rule_best[0].split(".")[1]  
    ...
    # ------- step : Data correlation Engine -> fetch historical data -------
    if dashboard_call==1:
        try: 
            historical_output = historical_retriever(incident, 6)
            incident['historical'] = historical_output
        except: incident['historical'] = []
        yield incident
    else:
        print("\n","-"*20, "Historical data Retriever", "-"*20)
        print("\n 🔄 Request sent to historical retriever: ", f"\n{GREEN}correlator_output{RESET} -> {correlator_output},\n {GREEN}situation_card{RESET} -> {incident['situation_card']}")
